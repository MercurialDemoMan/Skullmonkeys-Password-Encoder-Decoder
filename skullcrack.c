...
{
    /* 1st symbol */
    /* 1st bit */ 0x01, 0x00, /* if (bit == 1) then lives = lives | (0x01 << 0x00) */
    /* 2nd bit */ 0x01, 0x01, /* if (bit == 1) then lives = lives | (0x01 << 0x01) */
    /* 3rd bit */ 0x01, 0x02, /* if (bit == 1) then lives = lives | (0x01 << 0x02) */

    /* 2nd symbol */
    /* 1st bit */ 0x01, 0x03, /* if (bit == 1) then lives = lives | (0x01 << 0x03) */
    /* 2nd bit */ 0x00, 0x00, /* if (bit == 1) then level = level | (0x01 << 0x00) */
    /* 3rd bit */ 0x07, 0x00, /* if (bit == 1) then seed  = seed  | (0x01 << 0x00) */

    /* 3rd symbol */
    /* 1st bit */ 0x00, 0x01, /* if (bit == 1) then level = level | (0x01 << 0x01) */
    /* 2nd bit */ 0x07, 0x01, /* if (bit == 1) then seed  = seed  | (0x01 << 0x01) */
    /* 3rd bit */ 0x01, 0x04, /* if (bit == 1) then lives = lives | (0x01 << 0x04) */

    /* 4th symbol */
    /* 1st bit */ 0x06, 0x00, /* if (bit == 1) then 1970s = 1970s | (0x01 << 0x00) */
    /* 2nd bit */ 0x00, 0x02, /* if (bit == 1) then level = level | (0x01 << 0x02) */
    /* 3rd bit */ 0x07, 0x02, /* if (bit == 1) then seed  = seed  | (0x01 << 0x02) */

    /* 5th symbol */
    /* 1st bit */ 0x02, 0x00, /* if (bit == 1) then birds  = birds  | (0x01 << 0x00) */
    /* 2nd bit */ 0x03, 0x00, /* if (bit == 1) then pharts = pharts | (0x01 << 0x00) */
    /* 3rd bit */ 0x04, 0x00, /* if (bit == 1) then enemas = enemas | (0x01 << 0x00) */

    /* 6th symbol */
    /* 1st bit */ 0x05, 0x00, /* if (bit == 1) then willies = willies | (0x01 << 0x00) */
    /* 2nd bit */ 0x01, 0x05, /* if (bit == 1) then lives   = lives   | (0x01 << 0x05) */
    /* 3rd bit */ 0x07, 0x03, /* if (bit == 1) then seed    = seed    | (0x01 << 0x03) */

    /* 7th symbol */
    /* 1st bit */ 0x00, 0x03, /* if (bit == 1) then level = level   | (0x01 << 0x03) */
    /* 2nd bit */ 0x02, 0x01, /* if (bit == 1) then birds = birds   | (0x01 << 0x01) */
    /* 3rd bit */ 0x03, 0x01, /* if (bit == 1) then pharts = pharts | (0x01 << 0x01) */

    /* 8th symbol */
    /* 1st bit */ 0x04, 0x01, /* if (bit == 1) then enemas  = enemas  | (0x01 << 0x01) */
    /* 2nd bit */ 0x05, 0x01, /* if (bit == 1) then willies = willies | (0x01 << 0x01) */
    /* 3rd bit */ 0x06, 0x01, /* if (bit == 1) then 1970s   = 1970s   | (0x01 << 0x01) */

    /* 9th symbol */
    /* 1st bit */ 0x07, 0x04, /* if (bit == 1) then seed  = seed  | (0x01 << 0x04) */
    /* 2nd bit */ 0x01, 0x06, /* if (bit == 1) then lives = lives | (0x01 << 0x06) */
    /* 3rd bit */ 0x00, 0x04, /* if (bit == 1) then level = level | (0x01 << 0x04) */

    /* 10th symbol */
    /* 1st bit */ 0x07, 0x05, /* if (bit == 1) then seed   = seed   | (0x01 << 0x05) */
    /* 2nd bit */ 0x02, 0x02, /* if (bit == 1) then birds  = birds  | (0x01 << 0x02) */
    /* 3rd bit */ 0x03, 0x02, /* if (bit == 1) then pharts = pharts | (0x01 << 0x02) */

    /* 11th symbol */
    /* 1st bit */ 0x04, 0x02, /* if (bit == 1) then enemas  = enemas  | (0x01 << 0x02) */
    /* 2nd bit */ 0x05, 0x02  /* if (bit == 1) then willies = willies | (0x01 << 0x02) */
};
